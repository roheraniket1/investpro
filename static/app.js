class KotakNeoPro {
    ...
    renderSearchDropdown(data, targetDd = null, onSelect = null) {
        ...
        data.forEach(item => {
            ...
            const badgeColor = item.exchange === 'MCX' ? '#f59e0b' : (item.category === 'OPTION' ? '#a855f7' : (item.category === 'FUTURE' ? '#3b82f6' : '#10b981'));
            const ltpText = (item.ltp !== undefined && item.ltp !== null && item.ltp > 0) ? `₹${Number(item.ltp).toFixed(2)}` : '--';
            
            div.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 2px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span style="font-weight: 700; color: #fff; font-size: 0.95rem;">${item.symbol}</span>
                        <span style="font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; background: rgba(255,255,255,0.08); color: ${badgeColor}; border: 1px solid rgba(255,255,255,0.1); font-weight: 600;">${badge}</span>
                        <span style="font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; background: rgba(59,130,246,0.15); color: #60a5fa; font-weight: 600;">${item.exchange || 'NSE'}</span>
                    </div>
                    <div style="font-size: 0.78rem; color: #94a3b8; max-width: 320px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        ${item.name && item.name !== item.symbol ? item.name : item.trading_symbol}
                    </div>
                </div>
                <div style="text-align: right; display: flex; flex-direction: column; align-items: flex-end; gap: 2px;">
                    <span style="font-size: 0.95rem; font-weight: 700; color: var(--bullish-green);">${ltpText}</span>
                    <span style="font-size: 0.75rem; color: #64748b;">Lot: ${item.lot_size || 1}</span>
                </div>
            `;
